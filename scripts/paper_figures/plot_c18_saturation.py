...
for ax, r in zip(axes, D):
    ...
    for k, s in enumerate(SEEDS):
        traj(ax, z, r["t1"][f"high_{s:g}"], f"C{k}", label=SL[s])
    ax.axhline(0.5, color="k", lw=0.6); ax.set_yscale("log"); ax.set_xlim(25, 5); ax.set_ylim(1e-4, 1e2)
    ax.set_xlabel(r"$z$"); ax.set_title(r"$M_{\rm halo}(z{=}5)=$" + LAB[r["M0"]], fontsize=7.5)
axes[0].set_ylabel(r"$M_{\rm BH}/M_{\star,{\rm tot}}$"); axes[0].legend(title=r"seed [$M_\odot$]", fontsize=6.2, title_fontsize=6.2, loc="lower left")
axes[0].text(24.5, 0.6, r"$0.5$", fontsize=6.5)
fig.suptitle(r"High accessibility ($\sigma_j=1.5$, $R_{\rm nuc}=300$ pc): thick = median, band = 16--84\%, thin = 8 individual trees", fontsize=7, y=1.02)
fig.tight_layout(); fig.savefig(OUT / "sat_fig1_trajectories.png", dpi=250); plt.close(fig)

# Plot 2: final ratio vs seed mass
fig, axes = plt.subplots(1, 3, figsize=(7.2, 2.7), sharey=True)
for ax, r in zip(axes, D):
    for cfg, ls, col in (("high", "-", "C0"), ("fid", "--", "0.4")):
    ...
    ax.set_xlabel(r"$M_{\rm seed}\ [M_\odot]$"); ax.set_title(r"$M_{\rm halo}=$" + LAB[r["M0"]], fontsize=7.5)
axes[0].set_ylabel(r"$M_{\rm BH}/M_{\star,{\rm tot}}\,(z=5)$")
axes[0].legend([Line2D([0], [0], color="C0", lw=1.6), Line2D([0], [0], color="0.4", lw=1.6, ls="--")], [r"high accessibility", r"fiducial"], fontsize=6.2, loc="upper left")
fig.tight_layout(); fig.savefig(OUT / "sat_fig2_final_vs_seed.png", dpi=250); plt.close(fig)
...
for ax, r in zip(axes, D):
    ...
    ax.set_xlabel(r"$f_{\rm mom}$"); ax.set_title(r"$M_{\rm halo}=$" + LAB[r["M0"]], fontsize=7.5)
axes[0].set_ylabel(r"$M_{\rm BH}/M_{\star,{\rm tot}}\,(z=5)$"); axes[0].legend(title=r"seed [$M_\odot$]", fontsize=6.2, title_fontsize=6.2, loc="lower left")
fig.tight_layout(); fig.savefig(OUT / "sat_fig3_fmom.png", dpi=250); plt.close(fig)
...
for ax, r in zip(axes, D):
    ...
    ax.axhline(0.5, color="k", lw=0.6); ax.set_yscale("log"); ax.set_xlim(25, 5); ax.set_ylim(1e-4, 1e2)
    ax.set_xlabel(r"$z$"); ax.set_title(r"$M_{\rm halo}=$" + LAB[r["M0"]] + r", seed $10^3$", fontsize=7.5)
axes[0].set_ylabel(r"$M_{\rm BH}/M_{\star,{\rm tot}}$"); axes[0].legend(fontsize=6.2, loc="lower left")
fig.tight_layout(); fig.savefig(OUT / "sat_fig4_fmom_trajectories.png", dpi=250); plt.close(fig)
...
for ax, r in zip(axes, D):
    ...
    ax.set_xlabel(r"$z$"); ax.set_title(r"$M_{\rm halo}=$" + LAB[r["M0"]], fontsize=7.5)
axes[0].set_ylabel(r"$M_{\rm BH}/M_{\star,{\rm tot}}$"); axes[0].legend(fontsize=5.6, loc="lower left")
fig.suptitle(r"solid: seed $10^3$ (band and 8 trees), dashed: seed $10^7$ (median only)", fontsize=7, y=1.02)
fig.tight_layout(); fig.savefig(OUT / "sat_fig5_all_accessible.png", dpi=250); plt.close(fig)

...
The model behind the 2026 paper "Nuclear accessibility and competition for a shared gas reservoir in early black-hole growth". It is separate from the ODE model above (`main.py`) and is integrated along a halo's main-progenitor history on a uniform cosmic-time grid with an explicit, causally ordered step. **Frozen**; the production calculation and its provenance are in [`docs/PRODUCTION_PROVENANCE.md`](docs/PRODUCTION_PROVENANCE.md). The module docstring is the authoritative statement of what is and is not included.
